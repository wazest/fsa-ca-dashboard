        ...
        <div class="filters">
          ...
          <div class="filter-buttons">
            ...
            <button 
              ...
              :class="['filter-button', { active: selectedFilter === 'kickboxing' }]"
            >
              Kickboxing
            </button>
            <button 
            ...
            <button 
              ...
              :class="['filter-button', { active: selectedFilter === 'athletik' }]"
            >
              Athletik
            </button>
          </div>
...
const matchesFilter = (className: string): boolean => {
  ...
  if (selectedFilter.value === 'kickboxing') {
    return lowerClassName.includes('kickbox') || lowerClassName.includes('fitbox')
  }
  
  if (selectedFilter.value === 'bjj') {
    return lowerClassName.includes('bjj') || lowerClassName.includes('brazilian')
  }
  
  if (selectedFilter.value === 'athletik') {
    return lowerClassName.includes('athletik') || lowerClassName.includes('athletic')
  }
...
const overallAverageBookings = computed(() => {
  ...
  const sum = filteredClassAverages.value.reduce((total, cls) => total + cls.averageAttendance, 0)
  return Math.round((sum / filteredClassAverages.value.length) * 10) / 10
})

const highestAverageBookings = computed(() => {
  if (filteredClassAverages.value.length === 0) return 0
  const highest = Math.max(...filteredClassAverages.value.map(cls => cls.averageAttendance))
  return highest
})
    ...
    for (const file of Array.from(selectedFiles.value)) {
      ...
      const processedData = rawData.map((row: any) => {
        return {
          ...
          weekday: row['Tag'] || '',
          time: row['Zeit'] || '',
          averageAttendance: parseFloat(row['Durchschnittliche Teilnehmerzahl'] || '0'),
          date: timestamp
        }
